...
public class Main {
	...
	public static void stepTest() {
		for (int i = 0; i <= entitySet.length - 1; i++) {
			if (entitySet[i] == masterSquirrel | entitySet[i] == null) {
			} else if (entitySet[i].x == masterSquirrel.x && entitySet[i].y == masterSquirrel.y) {
				updateEnergy(entitySet[i].energy);
				deleteEntity(entitySet[i]);
			}
		}
	}

	public static void updateEnergy(int delta) {
		int x = masterSquirrel.energy + delta;
		masterSquirrel.setEnergy(x);
		entitySet[0]=masterSquirrel;
	}
}
